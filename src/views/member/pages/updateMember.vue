<template>
  <div class="detail">
    ...
    <div class="d_cont">
      ...
      <div class="a_btn">
        <mt-button type="primary" @click="updateMember">保存</mt-button>
        <mt-button type="primary" @click="quxiao">取消</mt-button>
      </div>
      <!--底部弹出框-->
...
<script type="text/babel">
  import { getMemberDetail,saveCameraPhoto,saveMember } from '../../../api/api';
  ...
  export default {
    ...
    created:function () {
      this.sexName="男";
      this.member.memberId=this.$route.query.memberId;
    ...
    methods: {
      ...
      getMemberDetail() {
        let params= {memberId:parseInt(this.member.memberId)};
        getMemberDetail(params).then((res)=>{
          if (res.code==100000 && res.data!=null) {
            this.member=res.data.member;
            if(this.member.gender==1){
              this.sexName="男"
            }else{
              this.sexName="女";
            }
            this.member.birthday=this.formatDate(this.member.birthday);
            this.gradeList = res.data.gradeList;
            for(var i=0;i<this.gradeList.length;i++){
                if(this.gradeList[i].mGradeId=this.member.mGroupId){
                  this.gradeName=this.gradeList[i].gradeName;
                }
            }
          }
        });
      ...
      updateMember(){
        saveMember(this.member).then(res=>{
          if(res.code==100000){
              this.$router.push({ path: '/member/data',query:{memberId:this.member.memberId}});
          }
        });
      },
